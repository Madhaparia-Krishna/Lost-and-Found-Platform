...
const FoundForm = ({ contact, setContact }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch('http://localhost:5000/found/submit', {
        method: 'POST',
        body: data,
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="form" encType="multipart/form-data" id="found-form">
      ...
      <label>
        Date found:
        <input name="date_found" type="date" value={formData.date_found} onChange={handleChange} required />
      </label>
      <label>
        Description:
